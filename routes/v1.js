...
router.get('/concours/:id', asyncHandler(async (req,res)=>{const item=await Contest.findOne(contestFilter(req.params.id)).populate('establishmentId educationLevelId programIds').lean();if(!item)throw new AppError(404,'CONTEST_NOT_FOUND','Concours introuvable');ok(res,toLegacyContest(item));}));
...
router.post('/concours',authenticate,requireSuperAdmin,required('libcnc','etablissement_id','niveau_id'),asyncHandler(async(req,res)=>{const input=await contestInput(req.body);input.slug=`${String(input.title).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'')}-${crypto.randomBytes(4).toString('hex')}`;const item=await Contest.create(input);ok(res,toLegacyContest(await Contest.findById(item._id).populate('establishmentId educationLevelId programIds').lean()),'Concours créé',201);}));
router.put('/concours/:id',authenticate,requireSuperAdmin,asyncHandler(async(req,res)=>{const item=await Contest.findOneAndUpdate(contestFilter(req.params.id),{$set:await contestInput(req.body)},{new:true,runValidators:true}).populate('establishmentId educationLevelId programIds').lean();if(!item)throw new AppError(404,'CONTEST_NOT_FOUND','Concours introuvable');ok(res,toLegacyContest(item),'Concours modifié');}));
