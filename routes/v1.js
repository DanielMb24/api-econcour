...
const { Contest, Program, ProgramSubject, Subject, DocumentRequirement, Application, ApplicationDocument, Payment, PaymentEvent, AuditLog, Message, Notification, Grade, Candidate, Establishment, Administrator, SupportRequest, Province, EducationLevel, Session } = require('../models/mongo');
...
const normalizeAcceptedMimeTypes = values => {
  const normalized = (Array.isArray(values) ? values : [])
    .map(normalizeMimeType)
    .filter(Boolean);
  return normalized.length ? [...new Set(normalized)] : ['application/pdf', 'image/jpeg', 'image/png', 'image/webp'];
};
...
const requirementView = item => ({id:String(item._id),code:item.code,nom:item.name,description:item.description||'',obligatoire:item.required,acceptedMimeTypes:normalizeAcceptedMimeTypes(item.acceptedMimeTypes),maxSizeBytes:item.maxSizeBytes});
...
const syncIncompleteDocumentsNotification = async (application, completion) => {
  ...
  if (!completion.complete) {
    const body = `Votre candidature ${application.nupcan} est incomplète. Téléversez ${completion.missing} pièce(s) obligatoire(s) restante(s) avant de procéder au paiement${completion.missingNames.length?`: ${completion.missingNames.join(', ')}`:''}.`;
    await Notification.findOneAndUpdate({applicationId:application._id,title,readAt:null},{$set:{candidateId:application.candidateId,legacyNupcan:application.nupcan,body,channel:'in_app'},$setOnInsert:{title}},{upsert:true,new:true});
  } else {
    await Notification.updateMany({applicationId:application._id,title,readAt:null},{$set:{readAt:new Date()}});
  }
};
router.get('/candidats/nipcan/:nipcan/dashboard', asyncHandler(async (req, res) => {
  ...
  const candidatures = await Promise.all(applications.map(async application => {
    ...
    const payment = payments.find(item => String(item.applicationId) === String(application._id));
    const completion = await documentCompletion(application);
    await syncIncompleteDocumentsNotification(application, completion);
    const documentsComplete = completion.complete;
    ...
    return {
      ...
      documents_requis: completion.required,
      documents_manquants: completion.missing,
      paiement_autorise: completion.complete || !!payment,
...
router.get('/dossiers/admin/all', authenticate, requirePermission('view_documents'), asyncHandler(async (_req, res) => {
  const [documents,paidApplications] = await Promise.all([ApplicationDocument.find().populate({ path: 'applicationId', populate: [{ path: 'candidateId' }, { path: 'contestId' }] }).sort({ createdAt: -1 }).lean(),Payment.find({status:'paid'}).distinct('applicationId')]);
  const paidIds=new Set(paidApplications.map(String));
  const transmitted=documents.filter(document=>paidIds.has(String(document.applicationId?._id)));
...
router.post('/dossiers', documentUpload.array('documents', 15), asyncHandler(async (req, res) => {
  ...
  req.files.forEach(assertReadableDocument);
  ...
  await autoCompleteFreePayment(application._id,req);
  ok(res, documents.map(documentView), 'Documents chargés', 201);
}));
router.post('/documents', documentUpload.single('file'), asyncHandler(async(req,res)=>{const nupcan=String(req.body?.nupcan||'').trim().toUpperCase();const application=await Application.findOne({nupcan}).lean();if(!application)throw new AppError(404,'APPLICATION_NOT_FOUND','Candidature introuvable');if(!req.file)throw new AppError(422,'DOCUMENT_REQUIRED','Un document est requis');assertReadableDocument(req.file);let requirement=null;if(req.body.requirement_id){requirement=await DocumentRequirement.findOne({_id:req.body.requirement_id,contestId:application.contestId,active:true,$or:[{programId:null},{programId:application.programId}]}).lean();if(!requirement)throw new AppError(422,'INVALID_DOCUMENT_REQUIREMENT','Ce document ne fait pas partie des pièces demandées pour ce concours et cette filière');}const file=req.file;const acceptedMimeTypes=normalizeAcceptedMimeTypes(requirement?.acceptedMimeTypes);if(requirement&&acceptedMimeTypes.length&&!acceptedMimeTypes.includes(normalizeMimeType(file.mimetype)))throw new AppError(422,'INVALID_DOCUMENT_TYPE',`Format de fichier non autorisé pour cette pièce. Formats acceptés: ${acceptedMimeTypes.join(', ')}`);if(requirement?.maxSizeBytes&&file.size>requirement.maxSizeBytes)throw new AppError(422,'DOCUMENT_TOO_LARGE','Le fichier dépasse la taille autorisée');const existing=requirement?await ApplicationDocument.findOne({applicationId:application._id,requirementId:requirement._id}):null;if(existing){existing.type=requirement?.name||String(req.body.nomdoc||existing.type||file.originalname);existing.required=requirement?.required||existing.required;existing.contentData=`data:${file.mimetype};base64,${file.buffer.toString('base64')}`;existing.originalName=file.originalname;existing.safeName=file.originalname.replace(/[^a-zA-Z0-9._-]/g,'_');existing.mimeType=file.mimetype;existing.size=file.size;existing.checksum=crypto.createHash('sha256').update(file.buffer).digest('hex');existing.status='uploaded';existing.rejectionReason=undefined;existing.version=(existing.version||1)+1;await existing.save();await audit(req,'DOCUMENT_REPLACED','ApplicationDocument',existing._id,{applicationId:String(application._id),version:existing.version,status:'uploaded',via:'post-upsert'});await autoCompleteFreePayment(application._id,req);return ok(res,documentView(existing),'Document remplacé et remis en attente');}const document=await ApplicationDocument.create({applicationId:application._id,candidateId:application.candidateId,requirementId:requirement?._id,type:requirement?.name||String(req.body.nomdoc||file.originalname),required:requirement?.required||false,storageKey:`documents/${nupcan}/${crypto.randomUUID()}`,contentData:`data:${file.mimetype};base64,${file.buffer.toString('base64')}`,originalName:file.originalname,safeName:file.originalname.replace(/[^a-zA-Z0-9._-]/g,'_'),mimeType:file.mimetype,size:file.size,checksum:crypto.createHash('sha256').update(file.buffer).digest('hex'),status:'uploaded'});await audit(req,'DOCUMENT_UPLOADED','ApplicationDocument',document._id,{applicationId:String(application._id),nupcan});await autoCompleteFreePayment(application._id,req);ok(res,documentView(document),'Document ajouté',201);}));
router.put('/documents/:id/replace', documentUpload.single('file'), asyncHandler(async(req,res)=>{if(!req.file)throw new AppError(422,'DOCUMENT_REQUIRED','Un document est requis');assertReadableDocument(req.file);const old=await ApplicationDocument.findById(req.params.id);if(!old)throw new AppError(404,'DOCUMENT_NOT_FOUND','Document introuvable');const application=await Application.findById(old.applicationId).select('contestId programId');if(!application)throw new AppError(404,'APPLICATION_NOT_FOUND','Candidature introuvable');const requirement=old.requirementId?await DocumentRequirement.findOne({_id:old.requirementId,contestId:application.contestId,active:true,$or:[{programId:null},{programId:application.programId}]}).lean():null;const file=req.file;const acceptedMimeTypes=normalizeAcceptedMimeTypes(requirement?.acceptedMimeTypes);if(requirement&&acceptedMimeTypes.length&&!acceptedMimeTypes.includes(normalizeMimeType(file.mimetype)))throw new AppError(422,'INVALID_DOCUMENT_TYPE',`Format de fichier non autorisé pour cette pièce. Formats acceptés: ${acceptedMimeTypes.join(', ')}`);if(requirement?.maxSizeBytes&&file.size>requirement.maxSizeBytes)throw new AppError(422,'DOCUMENT_TOO_LARGE','Le fichier dépasse la taille autorisée');old.type=requirement?.name||String(req.body.nomdoc||old.type);old.required=requirement?.required||old.required;old.contentData=`data:${file.mimetype};base64,${file.buffer.toString('base64')}`;old.originalName=file.originalname;old.safeName=file.originalname.replace(/[^a-zA-Z0-9._-]/g,'_');old.mimeType=file.mimetype;old.size=file.size;old.checksum=crypto.createHash('sha256').update(file.buffer).digest('hex');old.status='uploaded';old.rejectionReason=undefined;old.version=(old.version||1)+1;await old.save();await audit(req,'DOCUMENT_REPLACED','ApplicationDocument',old._id,{applicationId:String(old.applicationId),version:old.version,status:'uploaded'});ok(res,documentView(old),'Document remplacé et remis en attente');}));
const refreshApplicationStatus = async applicationId => {
  ...
  const nextStatus = nextApplicationStatus({hasRejected:rejected,allDocumentsApproved,paymentRequired,paymentPaid:paymentApproved,hasDocuments:documents.length>0});
  ...
  if (previousStatus !== nextStatus) application.statusHistory.push({ status: nextStatus, at: new Date() });
  ...
  if (nextStatus === 'approved' && previousStatus !== 'approved') {
    ...
    await Notification.create({candidateId:application.candidateId,applicationId:application._id,legacyNupcan:application.nupcan,title:'Candidature validée',body:`Félicitations, votre candidature ${application.nupcan} a été validée.`,channel:'in_app'});
    if (candidate?.email) emailService.sendCandidatureValidated({maican:candidate.email,prncan:candidate.firstName,nomcan:candidate.lastName,nupcan:application.nupcan}).catch(error=>console.error(JSON.stringify({level:'error',code:'APPLICATION_APPROVAL_EMAIL_FAILED',applicationId:String(application._id),message:error.message})));
  }
  return nextStatus;
};
router.put('/document-validation/:id', authenticate, requirePermission('validate_documents'), asyncHandler(async(req,res)=>{
  ...
  const currentDocument=await ApplicationDocument.findById(req.params.id).lean();if(!currentDocument)throw new AppError(404,'DOCUMENT_NOT_FOUND','Document introuvable');
  const application=await Application.findById(currentDocument.applicationId).populate('contestId');if(!application)throw new AppError(404,'APPLICATION_NOT_FOUND','Candidature introuvable');
  const paymentRequired=Number(application.contestId?.fee||application.contestSnapshot?.fee||0)>0;const paid=!paymentRequired||await Payment.exists({applicationId:application._id,status:'paid'});if(!paid)throw new AppError(409,'PAYMENT_REQUIRED','Le dossier doit être payé avant sa validation administrative');
  ...
  await audit(req,status==='approved'?'DOCUMENT_APPROVED':'DOCUMENT_REJECTED','ApplicationDocument',document._id,{applicationId:String(application._id),applicationStatus,commentaire:req.body.commentaire||''});
  if(status==='rejected')await Notification.create({candidateId:application.candidateId,applicationId:application._id,legacyNupcan:application.nupcan,title:'Document à remplacer',body:`La pièce « ${document.type} » a été rejetée${req.body.commentaire?`: ${String(req.body.commentaire).trim()}`:''}. Remplacez-la depuis votre dashboard. Aucun nouveau paiement ne sera demandé.`,channel:'in_app'});
...
router.post('/notes',authenticate,requirePermission('enter_grades'),required('candidat_id','concours_id','matiere_id','note'),asyncHandler(async(req,res)=>{const [candidate,contest,subject]=await Promise.all([Candidate.findOne(idFilter(req.body.candidat_id)).lean(),Contest.findOne(idFilter(req.body.concours_id)).lean(),Subject.findOne(idFilter(req.body.matiere_id)).lean()]);if(!candidate||!contest||!subject)throw new AppError(422,'INVALID_GRADE_SELECTION','Candidat, concours ou matière invalide');const application=await Application.findOne({candidateId:candidate._id,contestId:contest._id}).lean();if(!application)throw new AppError(404,'APPLICATION_NOT_FOUND','Candidature introuvable');const score=Number(req.body.note);if(!Number.isFinite(score)||score<0||score>20)throw new AppError(422,'INVALID_GRADE','La note doit être comprise entre 0 et 20');const grade=await Grade.findOneAndUpdate({applicationId:application._id,subjectId:subject._id},{$set:{score,maximumScore:20,coefficient:Number(req.body.coefficient||subject.coefficient||1),enteredBy:req.admin._id}},{upsert:true,new:true,runValidators:true});ok(res,{id:String(grade._id),note:grade.score},'Note enregistrée',201);}));
...
router.patch('/paiements/:id/status',authenticate,requirePermission('manage_payments'),asyncHandler(async(req,res)=>{const status={annule:'cancelled',rembourse:'refunded'}[req.body.statut]||req.body.status;if(!['cancelled','refunded'].includes(status))throw new AppError(422,'OPERATOR_CONFIRMATION_REQUIRED','Seul le webhook signé de l’opérateur peut confirmer ou échouer un paiement');const item=await Payment.findByIdAndUpdate(req.params.id,{$set:{status}},{new:true,runValidators:true});if(!item)throw new AppError(404,'PAYMENT_NOT_FOUND','Paiement introuvable');await audit(req,status==='refunded'?'PAYMENT_REFUNDED':'PAYMENT_CANCELLED','Payment',item._id,{applicationId:String(item.applicationId)});ok(res,{id:String(item._id),statut:item.status},'Statut du paiement modifié');}));
...
router.get('/applications/:nupcan/payment-eligibility', asyncHandler(async(req,res)=>{const application=await Application.findOne({nupcan:String(req.params.nupcan).trim().toUpperCase()}).populate('candidateId','firstName lastName').populate({path:'contestId',populate:{path:'establishmentId'}});if(!application)throw new AppError(404,'APPLICATION_NOT_FOUND','Candidature introuvable');const [completion,payment]=await Promise.all([documentCompletion(application),Payment.findOne({applicationId:application._id}).sort({createdAt:-1}).lean()]);await syncIncompleteDocumentsNotification(application,completion);let contestOpen=true,contestError=null;try{assertContestAcceptsPayment(application);}catch(error){contestOpen=false;contestError=error.message;}const eligible=(completion.complete&&contestOpen)||!!payment;ok(res,{eligible,contestOpen,contestError,alreadyPaid:payment?.status==='paid',hasPayment:!!payment,canRetry:['failed','cancelled'].includes(payment?.status),missing:completion.missing,missingNames:completion.missingNames,paymentContext:{nupcan:application.nupcan,candidat:{nomcan:application.candidateId?.lastName||'',prncan:application.candidateId?.firstName||''},concours:toLegacyContest(application.contestId)},payment:payment?{id:String(payment._id),statut:legacyPaymentStatus(payment.status),reference_paiement:payment.paymentReference,montant:payment.amount}:null},eligible?'Paiement autorisé':contestError||'Toutes les pièces obligatoires doivent être téléversées avant le paiement');}));
const transmitPaidApplication = async application => {application.status='submitted';application.paymentStatus='paid';application.submittedAt=application.submittedAt||new Date();application.statusHistory.push({status:'submitted',reason:'Paiement confirmé par webhook; dossier transmis aux agents',at:new Date()});await application.save();await Notification.findOneAndUpdate({applicationId:application._id,title:'Dossier transmis'},{$setOnInsert:{candidateId:application.candidateId,legacyNupcan:application.nupcan,title:'Dossier transmis',body:`Votre paiement pour ${application.nupcan} est confirmé. Votre dossier a été transmis aux agents pour validation administrative.`,channel:'in_app'}},{upsert:true,new:true});};
const autoCompleteFreePayment = async (applicationId,req) => {const application=await Application.findById(applicationId).populate('contestId');if(!application||Number(application.contestId?.fee??application.contestSnapshot?.fee??0)!==0)return null;assertContestAcceptsPayment(application);const completion=await documentCompletion(application);if(!completion.complete)return null;let payment=await Payment.findOne({applicationId:application._id});if(!payment)payment=await Payment.create({applicationId:application._id,candidateId:application.candidateId,provider:'development',paymentReference:`FREE-${application.nupcan}`,amount:0,currency:'XAF',status:'paid'});if(payment.status!=='paid'){payment.status='paid';await payment.save();}if(application.paymentStatus!=='paid'){await transmitPaidApplication(application);await audit(req,'FREE_PAYMENT_COMPLETED','Payment',payment._id,{applicationId:String(application._id)});}return payment;};
const initializePayment = async ({application,provider,currency,req}) => {
  assertContestAcceptsPayment(application);
  if(!['airtel_money','moov_money','cinetpay','development'].includes(provider))throw new AppError(422,'INVALID_PAYMENT_PROVIDER','Opérateur de paiement invalide');
  const fee=Number(application.contestId?.fee??application.contestSnapshot?.fee??0);
  let existing = await Payment.findOne({applicationId:application._id});
  const attemptAction=paymentAttemptAction(existing?.status);if(attemptAction==='reuse')return{payment:existing,existing:true,retried:false};
  const completion = await documentCompletion(application);
  await syncIncompleteDocumentsNotification(application,completion);
  if (!completion.complete) throw new AppError(409,'DOCUMENTS_INCOMPLETE',`Téléversez toutes les pièces obligatoires avant le paiement. ${completion.missing} pièce(s) manquante(s).`,completion.missingNames);
  const paymentReference=`PAY-${application.nupcan}-${crypto.randomUUID()}`;const status=fee===0?'paid':'pending';const retried=attemptAction==='retry';
  const payment=existing?await Payment.findByIdAndUpdate(existing._id,{$set:{provider,paymentReference,amount:fee,currency:currency||'XAF',status,transactionId:undefined}},{new:true,runValidators:true}):await Payment.create({applicationId:application._id,candidateId:application.candidateId,provider:fee===0?'development':provider,paymentReference,amount:fee,currency:currency||'XAF',status});
  application.paymentStatus=payment.status;await application.save();if(status==='paid')await transmitPaidApplication(application);
  await audit(req,retried?'PAYMENT_RETRIED':'PAYMENT_INITIATED','Payment',payment._id,{applicationId:String(application._id),provider:payment.provider,amount:fee,status});
  return {payment,existing:false,retried};
};
router.post('/paiements', required('nupcan','methode'), asyncHandler(async(req,res)=>{const application=await Application.findOne({nupcan:String(req.body.nupcan).trim().toUpperCase()}).populate('contestId');if(!application)throw new AppError(404,'APPLICATION_NOT_FOUND','Candidature introuvable');const provider={moov:'moov_money',airtel:'airtel_money',gorri:'development'}[req.body.methode]||req.body.methode;const result=await initializePayment({application,provider,currency:'XAF',req});ok(res,{id:String(result.payment._id),reference_paiement:result.payment.paymentReference,montant:result.payment.amount,methode:result.payment.provider,statut:legacyPaymentStatus(result.payment.status),existing:result.existing,retried:result.retried},result.existing?'Paiement existant réutilisé':result.retried?'Nouvelle tentative initialisée':'Paiement initialisé',result.existing?200:201);}));
router.post('/payments', required('applicationId','provider'), asyncHandler(async(req,res)=>{const application=await Application.findById(req.body.applicationId).populate('contestId');if(!application)throw new AppError(404,'APPLICATION_NOT_FOUND','Candidature introuvable');const result=await initializePayment({application,provider:req.body.provider,currency:req.body.currency,req});ok(res,{...result.payment.toObject(),existing:result.existing,retried:result.retried},result.existing?'Paiement existant réutilisé':'Paiement initialisé',result.existing?200:201);}));
router.post('/payments/webhook/:provider', asyncHandler(async(req,res)=>{
  const provider=String(req.params.provider).toLowerCase();const secret={airtel_money:process.env.AIRTEL_WEBHOOK_SECRET,moov_money:process.env.MOOV_WEBHOOK_SECRET,cinetpay:process.env.CINETPAY_WEBHOOK_SECRET}[provider];
  if(!secret)throw new AppError(503,'WEBHOOK_NOT_CONFIGURED','Webhook opérateur non configuré');
  const rawBody=req.rawBody||Buffer.from(JSON.stringify(req.body));const signature=String(req.get('x-payment-signature')||'');if(!verifyWebhookSignature(rawBody,signature,secret))throw new AppError(401,'INVALID_WEBHOOK_SIGNATURE','Signature du webhook invalide');
  const eventId=String(req.body.eventId||req.body.event_id||'').trim(),paymentReference=String(req.body.paymentReference||req.body.payment_reference||'').trim();if(!eventId||!paymentReference)throw new AppError(422,'INVALID_WEBHOOK_PAYLOAD','Identifiant d’événement et référence de paiement requis');
  const payment=await Payment.findOne({paymentReference,provider});if(!payment)throw new AppError(404,'PAYMENT_NOT_FOUND','Paiement introuvable');
  const duplicate=await PaymentEvent.findOne({provider,eventId}).lean();if(duplicate)return ok(res,{duplicate:true,status:payment.status},'Événement déjà traité');
  if(Number(req.body.amount)!==Number(payment.amount)||String(req.body.currency||payment.currency)!==payment.currency)throw new AppError(422,'PAYMENT_MISMATCH','Le montant ou la devise du webhook ne correspond pas au paiement');
  const nextStatus={success:'paid',successful:'paid',paid:'paid',failed:'failed',failure:'failed',cancelled:'cancelled',canceled:'cancelled',processing:'processing',pending:'pending'}[String(req.body.status||'').toLowerCase()];if(!nextStatus)throw new AppError(422,'INVALID_OPERATOR_STATUS','Statut opérateur inconnu');
  const before=payment.status;if(before!=='paid'){payment.status=nextStatus;payment.transactionId=String(req.body.transactionId||req.body.transaction_id||payment.transactionId||'')||undefined;await payment.save();}
  const payloadHash=crypto.createHash('sha256').update(req.rawBody||Buffer.from(JSON.stringify(req.body))).digest('hex');await PaymentEvent.create({paymentId:payment._id,provider,eventId,eventType:String(req.body.type||'payment.status'),signatureValid:true,payloadHash,statusBefore:before,statusAfter:payment.status,processedAt:new Date()});
  const application=await Application.findById(payment.applicationId);if(application){application.paymentStatus=payment.status;await application.save();if(payment.status==='paid'&&before!=='paid')await transmitPaidApplication(application);}
  await audit(req,'PAYMENT_WEBHOOK_PROCESSED','Payment',payment._id,{provider,eventId,statusBefore:before,statusAfter:payment.status,payloadHash});ok(res,{duplicate:false,status:payment.status},'Webhook traité');
}));
