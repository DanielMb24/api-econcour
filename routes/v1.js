...
router.post('/admin/auth/login', required('email','password'), asyncHandler(async (req,res)=>{
  ...
  ok(res,{token,admin:{id:String(admin._id),legacyId:admin.legacyId,nom:admin.lastName,prenom:admin.firstName,email:admin.email,role:admin.role,admin_role:admin.subAdminRole||(admin.role==='finance'?'paiements':'documents'),subAdminRole:admin.subAdminRole,permissions:effectivePermissions(admin),mustChangePassword:Boolean(admin.mustChangePassword),etablissement_id:establishment?.legacyId,etablissement_object_id:establishment?._id,etablissement_nom:establishment?.name}},'Connexion réussie');
}));
...
router.get('/notifications/unread',authenticate,asyncHandler(async(req,res)=>{const query={readAt:null};const ids=await scopedApplicationIds(req.admin);if(ids)query.applicationId={$in:ids};const items=await Notification.find(query).sort({createdAt:-1}).limit(20).lean();ok(res,items.map(item=>({...notificationView(item),link:'/admin/candidats'})),'Notifications administrateur chargées');}));
