...
const { Contest, Program, ProgramSubject, DocumentRequirement, Application, ApplicationDocument, Payment, PaymentEvent, AuditLog, Message, Notification, Grade, Candidate, Establishment, Administrator, SupportRequest, Province, EducationLevel, Session } = require('../models/mongo');
...
const documentView = d => ({ id: String(d._id), document_id: String(d._id), requirement_id: d.requirementId ? String(d.requirementId) : null, nomdoc: d.type, nom_fichier: d.originalName || d.safeName, chemin_fichier: d.storageKey, type: d.type, mime_type:d.mimeType, version:d.version||1, obligatoire: d.required, taille: d.size, statut: legacyDocumentStatus(d.status), commentaire_validation: d.rejectionReason || '', created_at: d.createdAt, updated_at: d.updatedAt });
